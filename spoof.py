...
class IntegratedVerificationSystem:
    ...
    def draw_verification_ui(self, frame):
        ...
        # Header
        cv2.putText(frame, "SISTEM VERIFIKASI ANTI-SPOOF", (20, 40), 
    ...
    def process_verification_stages(self, frame):
        ...
        # Deteksi wajah
        detected_name, confidence, face_location = self.recognize_face(frame)
    ...
    def run_verification(self):
        """Menjalankan proses verifikasi lengkap"""
        print("🚀 MEMULAI VERIFIKASI ANTI-SPOOF")
        print(f"Method: {'GazeTracking' if self.use_gaze_tracking else 'OpenCV Fallback'}")
        print("Tahapan: Deteksi → Pengenalan → Perintah → Verifikasi → Sukses")
        print("Tekan 'Q' untuk membatalkan\n")
        
        try:
            # Set window name yang unik
            window_name = 'Verifikasi Anti-Spoof'
    ...
    def cleanup(self):
        ...
        try:
            cv2.destroyAllWindows()
            # Wait untuk memastikan windows tertutup
            cv2.waitKey(1)
        except:
            pass
...
# Fungsi utama untuk integrasi dengan GUI
def run_complete_verification():
    """
    Fungsi untuk dipanggil dari main_window.py
    Returns: True jika berhasil, False jika gagal
    """
    try:
        print("Memulai sistem verifikasi terintegrasi...")
        system = IntegratedVerificationSystem()
        result = system.run_verification()
        
        if result:
            print("🎉 VERIFIKASI BERHASIL - Membuka sistem presensi")
        else:
